...
const App: React.FC = () => {
  ...
  const handleSlotClick = (eqId: number, timeId: string, date: string) => {
    ...
    // 1. If it's blocked, unavailable, pending, or approved -> Can't touch
    if (existingBooking) {
                    ...
                    <div className="flex items-center bg-slate-100 rounded-lg p-1">
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                                weekOffset === 0 ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                        >
                            Esta Semana
                        </button>
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                                weekOffset === 1 ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                        >
                            Próxima Semana
                        </button>
                                                                            ...
                                                                            <>
                                                                                {status === 'blocked' ? (
                                                                                    <div className="flex flex-col items-center opacity-60">
                                                                                        ...
                                                                                        <span className="text-center leading-tight text-[10px]">{booking.blockedReason || 'No disponible'}</span>
                                                                                    </div>
                                                                                ) : (
                                                                                    <div className="px-1">
